import type { WeatherResponse, TimeSeries } from '../response/weather-response';
...
export class WeatherForecast {
  constructor(weatherData: WeatherResponse) {
    ...
    this.precipitation_amount =
      weatherData.properties.timeseries[0].data.next_1_hours.details.precipitation_amount;
  ...
  getIntervals(timeSeries: TimeSeries[]): ForecastNextHours[] {
    ...
    const forecasts = intervalls
      .map((intervall) => {
        const match = timeSeries.find((t) => moment(t.time).hour() === intervall);
